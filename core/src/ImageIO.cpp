...
unsigned char *
ImageIO::loadFromMemoryRGBA32(const unsigned char *data, const size_t size,
                              size_t &width, size_t &height,
                              MaxSizeInfo *maxSize, Vector2i *baseSize,
                              Vector2i *packedSize, int subImageIndex) {
  ...
  if (fiMemory != nullptr) {
    ...
    if (format != FIF_UNKNOWN && FreeImage_FIFSupportsReading(format)) {
      ...
      if (subImageIndex < 0)
        ...
        fiMultiBitmap =
            FreeImage_LoadMultiBitmapFromMemory(format, fiMemory, GIF_PLAYBACK);
      ...
      if (fiBitmap != nullptr) {
        ...
        if (fiBitmap != nullptr) {
          ...
          size_t maxX =
              maxSize == nullptr ? 0 : (size_t)Math::round(maxSize->x());
          size_t maxY =
              maxSize == nullptr ? 0 : (size_t)Math::round(maxSize->y());

          if (maxSize != nullptr && maxX > 0 && maxY > 0 &&
              (width > maxX || height > maxY)) {
            Vector2i sz =
                adjustPictureSize(Vector2i(width, height), Vector2i(maxX, maxY),
                                  maxSize->externalZoom());

            if (sz.x() > Renderer::getScreenWidth() ||
                sz.y() > Renderer::getScreenHeight())
              sz = adjustPictureSize(sz,
                                     Vector2i(Renderer::getScreenWidth(),
                                              Renderer::getScreenHeight()),
                                     false);

            if (sz.x() != width || sz.y() != height) {
              LOG(LogDebug) << "ImageIO : rescaling image from "
                            << std::string(std::to_string(width) + "x" +
                                           std::to_string(height))
                                   .c_str()
                            << " to "
                            << std::string(std::to_string(sz.x()) + "x" +
                                           std::to_string(sz.y()))
                                   .c_str();

              FIBITMAP *imageRescaled =
                  FreeImage_Rescale(fiBitmap, sz.x(), sz.y(), FILTER_BOX);
          ...
          for (int y = (int)height; --y >= 0;) {
            unsigned int *argb =
                (unsigned int *)FreeImage_GetScanLine(fiBitmap, y);
...
void ImageIO::flipPixelsVert(unsigned char *imagePx, const size_t &width,
                             const size_t &height) {
  unsigned int temp;
...
Vector2f ImageIO::adjustPictureSizeF(Vector2f imageSize, Vector2f maxSize,
                                     bool externSize) {
  return adjustPictureSizeF(imageSize.x(), imageSize.y(), maxSize.x(),
                            maxSize.y(), externSize);
}

Vector2f ImageIO::adjustPictureSizeF(float cxDIB, float cyDIB, float iMaxX,
                                     float iMaxY, bool externSize) {
  if (externSize)
...
Vector2i ImageIO::adjustPictureSize(Vector2i imageSize, Vector2i maxSize,
                                    bool externSize) {
  if (externSize) {
    Vector2f szf = getPictureMinSize(Vector2f(imageSize.x(), imageSize.y()),
                                     Vector2f(maxSize.x(), maxSize.y()));
...
void ImageIO::loadImageCache() {
  ...
  while (std::getline(f, line)) {
    ...
    if (splits.size() == 4) {
      std::string file =
          Utils::FileSystem::resolveRelativePath(splits[0], relativeTo, true);
...
static bool _isCachablePath(const std::string &path) {
  return path.find("/themes/") == std::string::npos &&
...
void ImageIO::saveImageCache() {
  ...
  for (auto it : sizeCache) {
    ...
    std::string path =
        Utils::FileSystem::createRelativePath(it.first, relativeTo, true);
...
static bool extractSvgSize(const std::string &svgFilePath, float &width,
                           float &height) {
  width = -1;
  height = -1;
    ...
    if (viewBoxPos != std::string::npos) {
      ...
      if (viewBoxEnd != std::string::npos) {
        std::string viewBoxStr =
            line.substr(viewBoxPos, viewBoxEnd - viewBoxPos);
...
bool ImageIO::loadImageSize(const std::string &fn, unsigned int *x,
                            unsigned int *y) {
  {
    std::unique_lock<std::mutex> lock(sizeCacheLock);
...
#define BUFSIZE 30
  ...
  if (jfif && buf[0] == 0xFF && buf[1] == 0xD8 && buf[2] == 0xFF) {
    ...
    *x = (buf[9] << 8) + buf[10];
    ...
    LOG(LogDebug)
        << "ImageIO::loadImageSize\tJPG size "
        << std::string(std::to_string(*x) + "x" + std::to_string(*y)).c_str();
  ...
  // GIF: first three bytes say "GIF", next three give version number. Then
  // dimensions
  if (buf[0] == 'G' && buf[1] == 'I' && buf[2] == 'F') {
    ...
    *y = buf[8] + (buf[9] << 8);
    ...
    LOG(LogDebug)
        << "ImageIO::loadImageSize\tGIF size "
        << std::string(std::to_string(*x) + "x" + std::to_string(*y)).c_str();
  ...
  if (buf[0] == 0x89 && buf[1] == 'P' && buf[2] == 'N' && buf[3] == 'G' &&
    ...
    *y = (buf[20] << 24) + (buf[21] << 16) + (buf[22] << 8) + (buf[23] << 0);
    ...
    LOG(LogDebug)
        << "ImageIO::loadImageSize\tPNG size "
        << std::string(std::to_string(*x) + "x" + std::to_string(*y)).c_str();
...
bool ImageIO::getMultiBitmapInformation(const std::string &path,
                                        int &totalFrames, int &frameTime) {
  totalFrames = 1;
  frameTime = 0;
...
#if WIN32
  fileFormat =
      FreeImage_GetFileTypeU(Utils::String::convertToWideString(path).c_str());
#else
...
#if WIN32
  std::ifstream stream(Utils::String::convertToWideString(path),
                       std::ios::binary);
#else
  ...
  if (fiMemory != nullptr) {
    auto fiMultiBitmap =
        FreeImage_LoadMultiBitmapFromMemory(fileFormat, fiMemory);
    if (fiMultiBitmap != nullptr) {
      ...
      if (fiBitmap != nullptr) {
        ...
        if (totalFrames > 1) {
          ...
          FreeImage_GetMetadata(FIMD_ANIMATION, fiBitmap, "FrameTime",
                                &tagFrameTime);
          if (tagFrameTime != nullptr && FreeImage_GetTagCount(tagFrameTime))
            frameTime = *static_cast<const uint32_t *>(
                FreeImage_GetTagValue(tagFrameTime));
