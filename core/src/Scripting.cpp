...
static void executeCommandsThread() {
  while (true) {
    ...
    mScriptQueueEvent.wait(
        lock, []() { return mExitScriptQueue || !mScriptQueue.empty(); });
...
static void executeScript(const std::string &script,
                          const std::string &eventName, const std::string &arg1,
...
static std::set<std::string> _supportedExtensions = {".exe", ".cmd", ".bat",
                                                     ".ps1", ".sh",  ".py"};
...
void fireEvent(const std::string &eventName, const std::string &arg1,
  ...
  for (auto dir :
       VectorHelper::distinct(scriptDirList, [](auto x) { return x; })) {
    auto scripts = Utils::FileSystem::getDirContent(dir);
    for (auto script : scripts) {
#if WIN32
      auto ext =
          Utils::String::toLower(Utils::FileSystem::getExtension(script));
  ...
  // Process single scripts. This type of scripts are called with the event name
  // as 1st arg
  ...
  for (auto dir : VectorHelper::distinct(paths, [](auto x) { return x; })) {
    ...
    for (auto script : Utils::FileSystem::getDirectoryFiles(dir)) {
      ...
      auto ext =
          Utils::String::toLower(Utils::FileSystem::getExtension(script.path));
      if (_supportedExtensions.find(ext) == _supportedExtensions.cend())
        continue;
