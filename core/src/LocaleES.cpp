...
std::string EsLocale::changeLocale(const std::string &locale) {
  ...
  if (locale != "") {
    // this var is looked by gettext in priority (and it is set on most
    // environment, then change it to change the lang)
...
bool EsLocale::mCurrentLanguageLoaded =
    true; // By default, 'en' is considered loaded
...
PluralRule rules[] = {
    ...
    {"mk", "n==1||n%10==1?0:1",
     [](int n) { return n == 1 || n % 10 == 1 ? 0 : 1; }},
    {"is", "n%10!=1||n%100==11",
     [](int n) { return (n % 10 != 1 || n % 100 == 11) ? 1 : 0; }},
    {"lv", "n%10==1&&n%100!=11?0:n!=0?1:2",
     [](int n) { return n % 10 == 1 && n % 100 != 11 ? 0
                        : n != 0                     ? 1
                                                     : 2; }},
    ...
    {"kw", "n==1?0:n==2?1:n==3?2:3", [](int n) {
       return (n == 1) ? 0 : (n == 2) ? 1 : (n == 3) ? 2 : 3;
     }}};

PluralRule EsLocale::mPluralRule = rules[0];
const std::vector<PluralRule> pluralRules(rules, rules + sizeof(rules) /
                                                             sizeof(rules[0]));
...
const std::string EsLocale::getTextWithContext(const std::string &context,
...
const std::string EsLocale::nGetText(const std::string msgid,
                                     const std::string msgid_plural, int n) {
  if (mCurrentLanguage.empty() ||
      mCurrentLanguage == "en_US") // English default
...
void EsLocale::checkLocalisationLoaded() {
  ...
  for (auto file : {"es-features.po", "emulationstation2.po"}) {
    ...
    std::string xmlpath = ResourceManager::getInstance()->getResourcePath(
        ":/locale/" + mCurrentLanguage + "/" + file);
    if (!Utils::FileSystem::exists(xmlpath))
      xmlpath = ResourceManager::getInstance()->getResourcePath(
          ":/locale/" + mCurrentLanguage + "/LC_MESSAGES/" + file);

    if (!Utils::FileSystem::exists(xmlpath))
      xmlpath = ResourceManager::getInstance()->getResourcePath(
          ":/locale/lang/" + mCurrentLanguage + "/" + file);

    if (!Utils::FileSystem::exists(xmlpath))
      xmlpath = ResourceManager::getInstance()->getResourcePath(
          ":/locale/lang/" + mCurrentLanguage + "/LC_MESSAGES/" + file);
    ...
    if (!Utils::FileSystem::exists(xmlpath)) {
      ...
      if (shortNameDivider != std::string::npos) {
        ...
        xmlpath = ResourceManager::getInstance()->getResourcePath(
            ":/locale/" + shortName + "/" + file);
        if (!Utils::FileSystem::exists(xmlpath))
          xmlpath = ResourceManager::getInstance()->getResourcePath(
              ":/locale/" + shortName + "/LC_MESSAGES/" + file);

        if (!Utils::FileSystem::exists(xmlpath))
          xmlpath = ResourceManager::getInstance()->getResourcePath(
              ":/locale/lang/" + shortName + "/" + file);

        if (!Utils::FileSystem::exists(xmlpath))
          xmlpath = ResourceManager::getInstance()->getResourcePath(
              ":/locale/lang/" + shortName + "/LC_MESSAGES/" + file);
      }
    }

    if (!Utils::FileSystem::exists(xmlpath)) {
      xmlpath = ResourceManager::getInstance()->getResourcePath(
          ":/es_features.locale/" + mCurrentLanguage + "/" + file);
      if (!Utils::FileSystem::exists(xmlpath)) {
        auto shortNameDivider = mCurrentLanguage.find("_");
        if (shortNameDivider != std::string::npos)
          xmlpath = ResourceManager::getInstance()->getResourcePath(
              ":/es_features.locale/" +
              mCurrentLanguage.substr(0, shortNameDivider) + "/" + file);
      }
    }
    ...
    while (1) {
      ...
      if (line.find("\"Plural-Forms:") == 0) {
        ...
        if (start != std::string::npos) {
          ...
          //	if (Utils::String::startsWith(plural, "(") &&
          // Utils::String::endsWith(plural, ")")) 		plural =
          // plural.substr(1, plural.size() - 2);
          plural = Utils::String::replace(plural, "(", "");
          plural = Utils::String::replace(plural, ")", "");

          for (auto iter = pluralRules.cbegin(); iter != pluralRules.cend();
               iter++) {
            if (plural == iter->rule) {
              mPluralRule = *iter;
              break;
