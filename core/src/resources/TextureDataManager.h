...
// data should only have been constructed and not loaded for this to work
// correctly. When the get() function is called it indicates that a texture
// wants to be used so at this point the texture data is loaded (via a call to
// load()).
//
// Once the load is complete (which may not be on the first call to get() if the
// data is loaded in a background thread) then the get() function call
// uploadAndBind() to upload to VRAM if necessary and bind the texture. This is
// followed by a call to releaseRAM() which frees the memory buffer if the
// texture can be reloaded from disk if needed again
//
class TextureDataManager {
public:
  TextureDataManager();
  ...
  enum TextureLoadMode : int { ENABLED = 0, DISABLED = 1, MOVETOTOPONLY = 2 };

  std::shared_ptr<TextureData> add(const TextureResource *key, bool tiled,
                                   bool linear);

  // The texturedata being removed may be loading in a different thread. However
  // it will be referenced by a smart point so we only need to remove it from
  // our array and it will be deleted when the other thread has finished with it
  void remove(const TextureResource *key);

  void cancelAsync(const TextureResource *key);
  std::shared_ptr<TextureData>
  get(const TextureResource *key,
      TextureLoadMode enableLoading = TextureLoadMode::ENABLED);
  bool bind(const TextureResource *key);

  // Get the total size of all textures managed by this object, loaded and
  // unloaded in bytes
  size_t getTotalSize();
...
private:
  ...
  std::unordered_map<const TextureResource *,
                     std::list<std::shared_ptr<TextureData>>::const_iterator>
      mTextureLookup;
