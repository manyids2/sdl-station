...
TextureDataManager::TextureDataManager() { mLoader = new TextureLoader(this); }

TextureDataManager::~TextureDataManager() { delete mLoader; }

std::shared_ptr<TextureData> TextureDataManager::add(const TextureResource *key,
                                                     bool tiled, bool linear) {
  ...
  std::shared_ptr<TextureData> data =
      std::make_shared<TextureData>(tiled, linear);
...
TextureDataManager::get(const TextureResource *key,
                        TextureLoadMode enableLoading) {
  std::unique_lock<std::recursive_mutex> lock(mMutex);
...
bool compareTextures(const std::shared_ptr<TextureData> &first,
                     const std::shared_ptr<TextureData> &second) {
...
void TextureDataManager::cleanupVRAM(std::shared_ptr<TextureData> exclude) {
  ...
  size_t max_texture =
      (size_t)Settings::getInstance()->getInt("MaxVRAM") * 1024 * 1024;
...
TextureLoader::TextureLoader(TextureDataManager *mgr)
    : mManager(mgr), mExit(false) {
...
void TextureLoader::threadProc() {
  while (true) {
    ...
    mEvent.wait(lock, [this]() {
      return !paused && (mExit || !mTextureDataQ.empty());
    });
...
void TextureLoader::load(std::shared_ptr<TextureData> textureData) {
  ...
  if (mProcessingTextureDataQ.find(textureData) !=
      mProcessingTextureDataQ.cend())
    return;

  // Remove it from the queue if it is already there
  if (mTextureDataQSet.erase(textureData) > 0) {
    auto tx =
        std::find(mTextureDataQ.cbegin(), mTextureDataQ.cend(), textureData);
    if (tx != mTextureDataQ.cend())
      mTextureDataQ.erase(tx);
  }

  // Put it on the start of the queue as we want the newly requested textures to
  // load first
