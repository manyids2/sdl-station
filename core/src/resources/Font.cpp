...
Font::FontFace::FontFace(ResourceData &&d, int size) : data(d) {
  int err = FT_New_Memory_Face(sLibrary, data.ptr.get(), (FT_Long)data.length,
                               0, &face);
  if (!err)
...
size_t Font::getMemUsage() const {
  ...
  for (auto tex : mTextures)
    memUsage +=
        (tex->textureId != 0 ? tex->textureSize.x() * tex->textureSize.y() * 4
                             : 0);
...
Font::Font(int size, const std::string &path, bool menuScaling)
    : mSize(size), mPath(path) {
  mSize = size;
  // if(mSize > 160) mSize = 160; // maximize the font size while it is causing
  // issues on linux

  // GPI
  float scale = menuScaling ? Renderer::ScreenSettings::menuFontScale()
                            : Renderer::ScreenSettings::fontScale();
...
std::shared_ptr<Font> Font::get(int size, const std::string &path,
  ...
  std::pair<std::string, int> def(
      canonicalPath.empty() ? getDefaultPath() : canonicalPath, scaledSize);
  ...
  std::shared_ptr<Font> font =
      std::shared_ptr<Font>(new Font(size, def.first, menuScaling));
...
Font::FontTexture::~FontTexture() { deinitTexture(); }
...
bool Font::FontTexture::findEmpty(const Vector2i &size, Vector2i &cursor_out) {
  ...
  if (writePos.x() + size.x() >= textureSize.x() &&
    ...
    writePos = Vector2i(0, writePos.y() + rowHeight +
                               1); // leave 1px of space between glyphs
    rowHeight = 0;
  }
...
void Font::FontTexture::initTexture() {
  if (textureId == 0) {
    textureId =
        Renderer::createTexture(Renderer::Texture::ALPHA, true, false,
                                textureSize.x(), textureSize.y(), nullptr);
    if (textureId == 0)
      LOG(LogError) << "FontTexture::initTexture() failed to create texture "
                    << textureSize.x() << "x" << textureSize.y();
  }
}
...
void Font::getTextureForNewGlyph(const Vector2i &glyphSize,
  ...
  if (mTextures.size()) {
    ...
    LOG(LogDebug)
        << "Glyph texture cache full, creating a new texture cache for "
        << Utils::FileSystem::getFileName(mPath) << " " << mSize << "pt";
  }
  ...
  if (!ok) {
    LOG(LogError) << "Glyph too big to fit on a new texture (glyph size > "
                  << tex_out->textureSize.x() << ", "
                  << tex_out->textureSize.y() << ")!";
    delete tex;
    tex_out = NULL;
  }
}
...
std::vector<std::string> getFallbackFontPaths() {
  std::vector<std::string> fallbackFonts = {
      ...
      ":/Rubik-Regular.ttf"          // hebrew
                            // (https://fontmeme.com/polices/police-rubik-hebrew
                            // public domain)
  };
...
FT_Face Font::getFaceForChar(unsigned int id) {
  ...
  // look through our current font + fallback fonts to see if any have the glyph
  // we're looking for
  for (unsigned int i = 0; i < fallbackFonts.size() + 1; i++) {
    ...
    if (fit == mFaceCache.cend()) // doesn't exist yet
    {
      ...
      if (itCache == globalTTFCache.cend()) {
        ...
        globalTTFCache.insert(
            std::pair<std::string, ResourceData>(path, dataX));
      ...
      mFaceCache[i] = std::unique_ptr<FontFace>(
          new FontFace(std::move(itCache->second), mSize));
#else
      ResourceData data = ResourceManager::getInstance()->getFileData(path);
      mFaceCache[i] =
          std::unique_ptr<FontFace>(new FontFace(std::move(data), mSize));
#endif
  ...
  // nothing has a valid glyph - return the "real" face so we get a "missing"
  // character
...
Font::Glyph *Font::getGlyph(unsigned int id) {
  if (id < 255) {
    ...
    // When computing & displaying long descriptions in gamelist views, it can
    // come here textsize*2 times per frame
  ...
  if (!face) {
    LOG(LogError) << "Could not find appropriate font face for character " << id
                  << " for font " << mPath;
    return NULL;
  }
  ...
  if (FT_Load_Char(face, id, FT_LOAD_RENDER)) {
    LOG(LogError) << "Could not find glyph for character " << id << " for font "
                  << mPath << ", size " << mSize << "!";
    return NULL;
  }
  ...
  if (tex == NULL) {
    LOG(LogError) << "Could not create glyph for character " << id
                  << " for font " << mPath << ", size " << mSize
  ...
  pGlyph->texture = tex;
  pGlyph->texPos = Vector2f((float)cursor.x() / (float)tex->textureSize.x(),
                            (float)cursor.y() / (float)tex->textureSize.y());
  pGlyph->texSize =
      Vector2f((float)glyphSize.x() / (float)tex->textureSize.x(),
               (float)glyphSize.y() / (float)tex->textureSize.y());
  pGlyph->advance = Vector2f((float)g->metrics.horiAdvance / 64.0f,
                             (float)g->metrics.vertAdvance / 64.0f);
  pGlyph->bearing = Vector2f((float)g->metrics.horiBearingX / 64.0f,
  ...
  if (glyphSize.x() > 0 && glyphSize.y() > 0)
    Renderer::updateTexture(tex->textureId, Renderer::Texture::ALPHA,
                            cursor.x(), cursor.y(), glyphSize.x(),
                            glyphSize.y(), g->bitmap.buffer);

  // update max glyph height - Limit to ascii table. If we don't it can take in
  // the fallback fonts
...
// completely recreate the texture data for all textures based on mGlyphs
// information
...
void Font::renderSingleGlow(TextCache *cache, const Transform4x4f &parentTrans,
                            float x, float y, bool verticesChanged) {
...
void Font::renderTextCacheEx(TextCache *cache, const Transform4x4f &parentTrans,
                             unsigned int mGlowSize, unsigned int mGlowColor,
                             Vector2f &mGlowOffset, unsigned char mOpacity) {
  if ((mGlowColor & 0x000000FF) != 0 && mGlowSize > 0) {
    ...
    if (mGlowSize == 1 && mGlowOffset == Vector2f::Zero()) {
      ...
      cache->setColor((mGlowColor & 0xFFFFFF00) |
                      (unsigned char)(a * (mOpacity / 255.0)));
      ...
      cache->setColor(
          (mGlowColor & 0xFFFFFF00) |
          (unsigned char)((mGlowColor & 0xFF) * (mOpacity / 255.0)));
...
void Font::renderTextCache(TextCache *cache, bool verticesChanged) {
  ...
  for (auto &vertex : cache->vertexLists) {
    ...
    if (tex != 0) {
      vertex.verts[0].customShader =
          cache->customShader.path.empty() ? nullptr : &cache->customShader;
      Renderer::drawTriangleStrips(
          &vertex.verts[0], vertex.verts.size(), Renderer::Blend::SRC_ALPHA,
          Renderer::Blend::ONE_MINUS_SRC_ALPHA,
          cache->vertexLists.size() > 1 || verticesChanged);
  ...
  for (auto sub : cache->imageSubstitutes) {
    if (sub.texture && sub.texture->bind()) {
      if (Settings::DebugImage())
        Renderer::drawRect(sub.vertex[0].pos.x(), sub.vertex[0].pos.y(),
...
void Font::renderGradientTextCache(TextCache *cache, unsigned int colorTop,
                                   unsigned int colorBottom, bool horz) {
  if (cache == NULL) {
    LOG(LogError) << "Attempted to draw NULL TextCache!";
    return;
  }

  for (auto it = cache->vertexLists.cbegin(); it != cache->vertexLists.cend();
       it++) {
    ...
    for (int i = 0; i < it->verts.size(); i += 6) {
      ...
      const unsigned int colorT =
          Renderer::mixColors(colorTop, colorBottom, topPercent);
      const unsigned int colorB =
          Renderer::mixColors(colorTop, colorBottom, bottomPercent);
      ...
      // make duplicates of first and last vertex so this can be rendered as a
      // triangle strip
      vxs[i + 0] = vxs[i + 1];
      vxs[i + 5] = vxs[i + 4];
    }
...
bool isBidiChar(char c) { return (c & 0xE0) == 0xC0 && (c & 0x10) == 0x10; }
...
std::string tryFastBidi(const std::string &text) {
  ...
  while (i < text.length()) {
    ...
    if ((c & 0xE0) == 0xC0) {
      ...
      if (c == 32 && i + 1 < text.length() && isBidiChar(text[i + 1]) &&
          i > 1 && isBidiChar(text[i - 2])) {
        ret.insert(lastCursor, text.substr(i, 1));
        i++;
      } else {
        ret += text[i++];
...
Vector2f Font::sizeText(std::string text, float lineSpacing) {
  ...
  while (i < text.length()) {
    unsigned int character =
        Utils::String::chars2Unicode(text, i); // advances i
...
static inline bool isWhiteSpace(unsigned int c) {
  return c == (unsigned int)' ' || c == (unsigned int)'\n' ||
         c == (unsigned int)'\t' || c == (unsigned int)'\v' ||
         c == (unsigned int)'\f' || (unsigned int)c == '\r';
}

// Thanks eagle0wl'PR @ Retropie EmulationStation
// https://github.com/RetroPie/EmulationStation/pull/269/files Breaks up a
// normal string with newlines to make it fit xLen
std::string Font::wrapText(std::string text, float maxWidth) {
  ...
  while (text.length() > 0) // find next cut-point
  {
    ...
    if (cursor == text.length()) // arrived at end of text.
    {
      out += text;
      text.erase();
    } else // need to cut at last whitespace or lacking that, the previous
           // cursor.
    {
...
Vector2f Font::sizeWrappedText(std::string text, float xLen,
                               float lineSpacing) {
...
Vector2f Font::getWrappedTextCursorOffset(std::string text, float xLen,
  ...
  size_t cursor = 0;
  while (cursor < stop) {
    unsigned int wrappedCharacter =
        Utils::String::chars2Unicode(wrappedText, wrapCursor);
...
TextCache *Font::buildTextCache(const std::string &_text, Vector2f offset,
                                unsigned int color, float xLen,
                                Alignment alignment, float lineSpacing) {
  float x = offset[0] +
            (xLen != 0 ? getNewlineStartOffset(_text, 0, xLen, alignment) : 0);
  ...
  if (alignment == ALIGN_LEFT && text.find("\t") != std::string::npos) {
    for (auto line : Utils::String::split(text, '\n', true)) {
      ...
      while (pos < text.length()) {
        unsigned int character =
            Utils::String::chars2Unicode(text, pos); // also advances cursor
  ...
  while (cursor < text.length()) {
    unsigned int character =
        Utils::String::chars2Unicode(text, cursor); // also advances cursor

    auto it = substituableChars.find(character);
    if (it != substituableChars.cend() &&
        ResourceManager::getInstance()->fileExists(it->second)) {
      auto padding = (yTop / 4.0f);

      // MaxSizeInfo mx(yBot - (2.0f * padding), yBot - (2.0f * padding));

      TextImageSubstitute is;
      is.texture = TextureResource::get(it->second, true, true, true, false,
                                        true /*, &mx*/);
      if (is.texture != nullptr) {
        Renderer::Rect rect(x, y - yDecal + padding, yBot - (2.0f * padding),
                            yBot - padding);

        auto imgSize = is.texture->getPhysicalSize();
        auto sz = ImageIO::adjustPictureSize(Vector2i(imgSize.x(), imgSize.y()),
                                             Vector2i(rect.w, rect.h));

        Renderer::Rect rc(rect.x + (rect.w / 2.0f) - (sz.x() / 2.0f),
                          rect.y + (rect.h / 2.0f) - (sz.y() / 2.0f), sz.x(),
                          sz.y());

        unsigned int vertexColor =
            Renderer::convertColor(0xFFFFFF00 | (color & 0xFF));

        is.vertex[0] = {
            {(float)rc.x, (float)rc.y + rc.h}, {0.0f, 0.0f}, vertexColor};
        is.vertex[1] = {{(float)rc.x, (float)rc.y}, {0.0f, 1.0f}, vertexColor};
        is.vertex[2] = {{(float)rc.x + rc.w, (float)rc.y + rc.h},
                        {1.0f, 0.0f},
                        vertexColor};
        is.vertex[3] = {
            {(float)rc.x + rc.w, (float)rc.y}, {1.0f, 1.0f}, vertexColor};
    ...
    vertices[1] = {{glyphStartX, y - glyph->bearing.y()},
                   ...
                   convertedColor};
    vertices[2] = {
        {glyphStartX, y - glyph->bearing.y() + (glyph->glyphSize.y())},
        {glyph->texPos.x(), glyph->texPos.y() + glyph->texSize.y()},
        convertedColor};
    vertices[3] = {{glyphStartX + glyph->glyphSize.x(), y - glyph->bearing.y()},
                   {glyph->texPos.x() + glyph->texSize.x(), glyph->texPos.y()},
                   convertedColor};
    vertices[4] = {{glyphStartX + glyph->glyphSize.x(),
                    y - glyph->bearing.y() + (glyph->glyphSize.y())},
...
//
//   float sh =
//       (float)Math::min(Renderer::getScreenHeight(),
...
void Font::OnThemeChanged() {
  static std::map<unsigned int, std::string> defaultMap = {
      {0xF300, ":/flags/au.png"}, {0xF301, ":/flags/br.png"},
      {0xF302, ":/flags/ca.png"}, {0xF303, ":/flags/ch.png"},
      {0xF304, ":/flags/de.png"}, {0xF305, ":/flags/es.png"},
      {0xF306, ":/flags/eu.png"}, {0xF307, ":/flags/fr.png"},
      {0xF308, ":/flags/gr.png"}, {0xF309, ":/flags/in.png"},
      {0xF30A, ":/flags/it.png"}, {0xF30B, ":/flags/jp.png"},
      {0xF30C, ":/flags/kr.png"}, {0xF30D, ":/flags/nl.png"},
      {0xF30E, ":/flags/no.png"}, {0xF30F, ":/flags/pt.png"},
      {0xF310, ":/flags/ru.png"}, {0xF311, ":/flags/sw.png"},
      {0xF312, ":/flags/uk.png"}, {0xF313, ":/flags/us.png"},
      {0xF314, ":/flags/wr.png"}};
  ...
  for (auto testPath : paths) {
    ...
    for (auto file : Utils::FileSystem::getDirectoryFiles(fontoverrides)) {
      ...
      if (Utils::String::toLower(Utils::FileSystem::getExtension(file.path)) !=
          ".png")
        continue;
      ...
      if (val >= 0xF000) {
        ...
        substituableChars.insert(
            std::pair<unsigned int, std::string>(val, file.path));
      }
    }
  }
}
