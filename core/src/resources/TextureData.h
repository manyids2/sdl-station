...
public:
  ...
  virtual std::vector<std::string> extractPdfImages(const std::string &fileName,
                                                    int pageIndex = -1,
                                                    int pageCount = 1,
                                                    int quality = 0) = 0;
};
...
public:
  ...
  //!!!! Needs to be canonical path. Caller should check for duplicates before
  //! calling this
  ...
  bool initImageFromMemory(const unsigned char *fileData, size_t length,
                           int subImageIndex = -1);
  bool initFromRGBA(unsigned char *dataRGBA, size_t width, size_t height,
                    bool copyData = true);
  ...
  // Upload the texture to VRAM if necessary and bind. Returns true if bound ok
  // or false if either not loaded
  bool uploadAndBind();
  ...
  inline size_t getVRAMUsage() {
    return mTextureID != 0 || mDataRGBA != nullptr ? mSize.x() * mSize.y() * 4
                                                   : 0;
  }
  ...
  void setPhysicalSize(int width, int height) {
    mPhysicalSize.x() = width;
    mPhysicalSize.y() = height;
  }
  ...
  bool updateFromExternalRGBA(unsigned char *dataRGBA, size_t width,
                              size_t height);
