...
TextureData::TextureData(bool tile, bool linear)
    : mTile(tile), mLinear(linear), mTextureID(0), mDataRGBA(nullptr),
      mScalable(false), mDynamic(true), mReloadable(false),
      mSize(Vector2i::Zero()), mPhysicalSize(Vector2f::Zero()),
      mMaxSize(MaxSizeInfo::Empty) {
...
// bool TextureData::initSVGFromMemory(const unsigned char *fileData,
//                                     size_t length) {
...
//
//     if (!mMaxSize.empty() && sourceWidth < mMaxSize.x() &&
//         sourceHeight < mMaxSize.y()) {
//       auto sz =
//           ImageIO::adjustPictureSizeF(sourceWidth, sourceHeight,
//           mMaxSize.x(),
//                                       mMaxSize.y(), mMaxSize.externalZoom());
//       sourceHeight = sz.y();
//       sourceWidth =
//           (sourceHeight * svgImage->width) /
//           svgImage
//               ->height; // FCA : Always compute width using source aspect
//               ratio
//     }
//   } else {
...
//         (sourceHeight * svgImage->width) /
//         svgImage
//             ->height; // FCA : Always compute width using source aspect ratio
//   }
//
...
//     width = (size_t)Math::round(((float)height / svgImage->height) *
//                                 svgImage->width);
//   } else if (height == 0) {
//     // auto scale height to keep aspect
//     height = (size_t)Math::round(((float)width / svgImage->width) *
//                                  svgImage->height);
//   }
//
//   if (OPTIMIZEVRAM && !mMaxSize.empty() &&
//       (width > mMaxSize.x() || height > mMaxSize.y())) {
//     auto imageSize = Vector2i(width, height);
//     auto displaySize = Vector2i((int)Math::round(mMaxSize.x()),
//                                 (int)Math::round(mMaxSize.y()));
//
//     Vector2i sz = ImageIO::adjustPictureSize(imageSize, displaySize,
...
//   nsvgRasterize(rast, svgImage, 0, 0, scale, dataRGBA, (int)width,
//   (int)height,
//                 (int)width * 4);
...
bool TextureData::initImageFromMemory(const unsigned char *fileData,
                                      size_t length, int subImageIndex) {
  // If already initialised then don't read again
  if (isLoaded())
    return true;

  // Don't load images greater than screen resolution
  MaxSizeInfo maxSize(Renderer::getScreenWidth(), Renderer::getScreenHeight(),
                      false);
  if (!mMaxSize.empty() && mMaxSize.x() < maxSize.x() &&
      mMaxSize.y() < maxSize.y())
    maxSize = mMaxSize;
  ...
  unsigned char *imageRGBA = ImageIO::loadFromMemoryRGBA32(
      (const unsigned char *)(fileData), length, width, height, &maxSize,
      &physicalSize, &size, subImageIndex);
...
bool TextureData::initFromRGBA(unsigned char *dataRGBA, size_t width,
                               size_t height, bool copyData) {
  // If already initialised then don't read again
  std::unique_lock<std::mutex> lock(mMutex);
...
bool TextureData::updateFromExternalRGBA(unsigned char *dataRGBA, size_t width,
                                         size_t height) {
  // If already initialised then don't read again
  std::unique_lock<std::mutex> lock(mMutex);
  ...
  if (mTextureID != 0)
    Renderer::updateTexture(mTextureID, Renderer::Texture::RGBA, 0, 0, width,
                            height, mDataRGBA);
...
//
//   auto val = Utils::FileSystem::createRelativePath(
//       Utils::FileSystem::changeExtension(mPath, ".jpg"),
//       Paths::getHomePath(), true);
//   val = Utils::String::replace(val, "~/../", "./");
//
//   std::string localFile = Utils::FileSystem::resolveRelativePath(
//       val, Paths::getUserEmulationStationPath() + "/tmp/videothumbs/", true);
//
...
//       return false;
//
//     vlcMedia = libvlc_media_new_path(
//         vlcInstance, Utils::FileSystem::getPreferredPath(mPath).c_str());
//     if (vlcMedia == nullptr) {
...
//
//     int result =
//         libvlc_video_take_snapshot(vlcMediaPlayer, 0, localFile.c_str(), 0,
//         0);
//
...
//
//     if (initImageFromMemory((const unsigned char *)data.ptr.get(),
//                             data.length)) {
//       ImageIO::updateImageCache(mPath, Utils::FileSystem::getFileSize(mPath),
//                                 Math::round((int)mPhysicalSize.x()),
//                                 Math::round((int)mPhysicalSize.y()));
//       return true;
//     }
//   }
//
...
bool TextureData::loadFromCbz() {
  ...
  if (zipFile.load(mPath)) {
    for (auto file : zipFile.infolist()) {
      auto ext = Utils::String::toLower(
          Utils::FileSystem::getExtension(file.filename));
      if (ext != ".jpg")
        continue;
    ...
    std::sort(files.begin(), files.end(),
              [](const Utils::Zip::ZipInfo &a, const Utils::Zip::ZipInfo &b) {
                return Utils::String::toLower(a.filename) <
                       Utils::String::toLower(b.filename);
              });
  }
  ...
  if (files.size() > 0 && files[0].file_size > 0) {
    ...
    Utils::Zip::zip_callback func = [](void *pOpaque, unsigned long long ofs,
...
bool TextureData::load(bool updateCache) {
  ...
  std::string ext =
      Utils::String::toLower(Utils::FileSystem::getExtension(mPath));
  ...
  if (updateCache && retval)
    ImageIO::updateImageCache(mPath, data.length,
                              Math::round((int)mPhysicalSize.x()),
                              Math::round((int)mPhysicalSize.y()));
...
bool TextureData::uploadAndBind() {
  ...
  if (mTextureID != 0)
    ...
    // Upload texture
    mTextureID =
        Renderer::createTexture(Renderer::Texture::RGBA, mLinear, mTile,
                                mSize.x(), mSize.y(), mDataRGBA);
    if (mTextureID == 0)
      return false;
...
void TextureData::setMaxSize(const MaxSizeInfo &maxSize) {
  ...
  if (mPhysicalSize.empty())
    ...
    auto imageSize = Vector2i((int)Math::round(mPhysicalSize.x()),
                              Math::round(mPhysicalSize.y()));

    Vector2i value = ImageIO::adjustPictureSize(
        imageSize, Vector2i(mMaxSize.x(), mMaxSize.y()),
        mMaxSize.externalZoom());
    Vector2i newVal = ImageIO::adjustPictureSize(
        imageSize, Vector2i(maxSize.x(), maxSize.y()), mMaxSize.externalZoom());
...
bool TextureData::isMaxSizeValid() {
  ...
  if ((int)Math::round(mPhysicalSize.x()) == mSize.x() &&
      (int)Math::round(mPhysicalSize.y()) == mSize.y())
    return true;
...
bool TextureData::rasterizeAt(float width, float height) {
  ...
  if (mScalable) {
    if ((int)Math::round(mScalableMinimumSize.y()) < h &&
        (int)Math::round(mScalableMinimumSize.x()) != w) {
      LOG(LogDebug)
          << "Requested SVG image size too small. Reloading image from ("
          << mScalableMinimumSize.x() << ", " << mScalableMinimumSize.y()
          << ") to (" << width << ", " << height << ")";
  ...
  } else if (!mMaxSize.empty() && h < Renderer::getScreenHeight() &&
             w < Renderer::getScreenWidth() && OPTIMIZEVRAM) {
    if (mSize.y() < h && mSize.x() < w &&
        (int)Math::round(mPhysicalSize.y()) >= h &&
        (int)Math::round(mPhysicalSize.x()) > w) {
      LOG(LogDebug)
          << "Requested PNG/JPG image size too small. Reloading image from ("
          << mSize.x() << ", " << mSize.y() << ") to (" << width << ", "
          << height << ")";
