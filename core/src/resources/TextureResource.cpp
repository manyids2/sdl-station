...
TextureResource::TextureResource(const std::string &path, bool tile,
                                 bool linear, bool dynamic, bool allowAsync,
                                 const MaxSizeInfo *maxSize)
    : mTextureData(nullptr), mForceLoad(false) {
  // Create a texture data object for this texture
  if (!path.empty()) {
    // If there is a path then the 'dynamic' flag tells us whether to use the
    // texture data manager to manage loading/unloading of this texture
    ...
    if (dynamic) {
      ...
      if (allowAsync && Settings::getInstance()->getBool("AsyncImages") &&
          ImageIO::loadImageSize(
              ResourceManager::getInstance()->getResourcePath(path), &width,
              &height)) {
        data->setScalable(Utils::FileSystem::isSVG(path));
        data->setStoredSize(width, height);
        data->setPhysicalSize(width, height);

        if (maxSize != nullptr && !maxSize->empty() &&
            Settings::getInstance()->getBool("OptimizeVRAM")) {
          auto sz = ImageIO::adjustPictureSize(
              Vector2i(width, height),
              Vector2i((int)Math::round(maxSize->x()),
                       (int)Math::round(maxSize->y())),
  ...
  if (mTextureData != nullptr && sNonDynamicTextureResources.find(this) ==
                                     sNonDynamicTextureResources.end())
...
TextureResource::get(const std::string &path, bool tile, bool linear,
                     bool forceLoad, bool dynamic, bool asReloadable,
                     const MaxSizeInfo *maxSize, const std::string &shareId) {
  std::shared_ptr<ResourceManager> &rm = ResourceManager::getInstance();

  const std::string canonicalPath = Utils::FileSystem::getCanonicalPath(path);
  if (canonicalPath.empty()) {
    std::shared_ptr<TextureResource> tex =
        std::make_shared<TextureResource>("", tile, linear, false, false);
    rm->addReloadable(tex); // make sure we get properly deinitialized even
                            // though we do nothing on reinitialization
    return tex;
  }
    ...
    if (!foundTexture->second.expired()) {
      ...
      if (maxSize != nullptr && !maxSize->empty() &&
        ...
        if (rc->mTextureData != nullptr)
          ...
          dt = sTextureDataManager.get(
              rc.get(), TextureDataManager::TextureLoadMode::DISABLED);
  ...
  // need to create it
  std::shared_ptr<TextureResource> tex;
  tex = std::make_shared<TextureResource>(std::get<0>(key), tile, linear,
                                          dynamic, !forceLoad, maxSize);

  auto loadMode = forceLoad ? TextureDataManager::TextureLoadMode::ENABLED
                            : TextureDataManager::TextureLoadMode::DISABLED;
  std::shared_ptr<TextureData> data =
      sTextureDataManager.get(tex.get(), loadMode);
  ...
  // Force load it if necessary. Note that it may get dumped from VRAM if we run
  // low
  if (forceLoad) {
...
void TextureResource::updateFromExternalPixels(unsigned char *dataRGBA,
...
void TextureResource::initFromPixels(unsigned char *dataRGBA, size_t width,
                                     size_t height) {
  // This is only valid if we have a local texture data object
  if (mTextureData == nullptr)
    return;
  ...
  // FCA optimisation, if streamed image size is already the same, don't
  // free/reallocate memory (which is slow), just copy bytes
  if (mTextureData->getDataRGBA() != nullptr && mSize.x() == width &&
      mSize.y() == height) {
...
bool TextureResource::isTiled() const {
  auto data = mTextureData
                  ? mTextureData
                  : sTextureDataManager.get(
                        this, TextureDataManager::TextureLoadMode::DISABLED);
...
void TextureResource::prioritize() const {
  if (mTextureData == nullptr)
    sTextureDataManager.get(this,
                            TextureDataManager::TextureLoadMode::MOVETOTOPONLY);
}
...
void TextureResource::setRequired(bool value) const {
  ...
  auto data = sTextureDataManager.get(
      this, TextureDataManager::TextureLoadMode::DISABLED);
  if (data != nullptr)
...
// For scalable source images in textures we want to set the resolution to
// rasterize at
...
void TextureResource::reload() {
  // For dynamically loaded textures the texture manager will load them on
  // demand. For manually loaded textures we have to reload them here
  if (mTextureData) {
...
void TextureResource::clearQueue() { sTextureDataManager.clearQueue(); }

const Vector2i TextureResource::getSize() const { return mSize; }
