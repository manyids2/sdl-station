  ...
  if (_cachedPaths.size() == 0 || _cachedThemeSet != themeSet) {
    ...
    if (!Paths::getUserThemesPath().empty()) {
      std::string themePath =
          Paths::getUserThemesPath() + "/" + themeSet + "/resources";
      if (Utils::FileSystem::isDirectory(themePath))
        _cachedPaths.push_back(themePath);
    }

    if (!Paths::getThemesPath().empty()) {
      std::string roThemePath =
          Paths::getThemesPath() + "/" + themeSet + "/resources";
    ...
    if (std::find(_cachedPaths.cbegin(), _cachedPaths.cend(), cwd) ==
        _cachedPaths.cend())
...
std::string ResourceManager::getResourcePath(const std::string &path) const {
  ...
  if (Utils::String::startsWith(path, ":/locale/") ||
      Utils::String::startsWith(path, ":/es_features.locale/")) {
    std::string test = Utils::FileSystem::getCanonicalPath(
        Paths::getEmulationStationPath() + "/" + &path[2]);
    if (Utils::FileSystem::exists(test)) {
      _resourcePathCache.insert(path, test);
      return test;
    }

    test = Utils::FileSystem::getCanonicalPath(
        Paths::getUserEmulationStationPath() + "/" + &path[2]);
    if (Utils::FileSystem::exists(test)) {
      _resourcePathCache.insert(path, test);
      return test;
    }
...
bool ResourceManager::fileExists(const std::string &path) const {
  // Animated Gifs : Check if the extension contains a ',' -> If it's the case,
  // we have the multi-image index as argument
