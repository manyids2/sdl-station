...
public:
  ...
  explicit Vector2f(const Vector3f &_v)
      : mX(((Vector2f &)_v).mX), mY(((Vector2f &)_v).mY) {}
  ...
  const Vector2f operator+(const float &_other) const {
    return {mX + _other, mY + _other};
  }
  const Vector2f operator-(const float &_other) const {
    return {mX - _other, mY - _other};
  }
  ...
  Vector2f &operator+=(const float &_other) {
    *this = *this + _other;
    return *this;
  }
  Vector2f &operator-=(const float &_other) {
  ...
  float &operator[](const int _index) {
    assert(_index < 2 && "index out of range");
    return (&mX)[_index];
  }
  ...
  Vector2f &round();
  Vector2f &lerp(const Vector2f &_start, const Vector2f &_end,
