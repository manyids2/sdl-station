...
float cosf(const float _num) { return ::cosf(_num); } // Math::cos

float sinf(const float _num) { return ::sinf(_num); } // Math::sin

float floorf(const float _num) { return ::floorf(_num); } // Math::floor

float ceilf(const float _num) { return ::ceilf(_num); } // Math::ceil
...
float bounce(const float _delayTime, const float _scrollTime,
  ...
  if (_currentTime < _delayTime) {
    ...
    return _scrollLength;
             ...
             (_delayTime + _scrollTime + _delayTime + _scrollTime)) {
