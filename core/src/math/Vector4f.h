...
public:
  ...
  explicit Vector4f(const Vector3f &_v)
      : mX(((Vector4f &)_v).mX), mY(((Vector4f &)_v).mY),
        mZ(((Vector4f &)_v).mZ), mW(0) {}
  explicit Vector4f(const Vector3f &_v, const float _w)
      : mX(((Vector4f &)_v).mX), mY(((Vector4f &)_v).mY),
        mZ(((Vector4f &)_v).mZ), mW(_w) {}
  ...
  Vector4f &operator+=(const Vector4f &_other) {
    *this = *this + _other;
    return *this;
  }
  Vector4f &operator-=(const Vector4f &_other) {
  ...
  Vector4f &operator+=(const float &_other) {
    *this = *this + _other;
    return *this;
  }
  Vector4f &operator-=(const float &_other) {
  ...
  float &operator[](const int _index) {
    assert(_index < 4 && "index out of range");
    return (&mX)[_index];
  }
  ...
  Vector4f &round();
  Vector4f &lerp(const Vector4f &_start, const Vector4f &_end,
