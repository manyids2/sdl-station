...
public:
  ...
  explicit Vector3f(const Vector4f &_v)
      : mX(((Vector3f &)_v).mX), mY(((Vector3f &)_v).mY),
        mZ(((Vector3f &)_v).mZ) {}
  ...
  Vector3f &operator+=(const float &_other) {
    *this = *this + _other;
    return *this;
  }
  Vector3f &operator-=(const float &_other) {
  ...
  float &operator[](const int _index) {
    assert(_index < 3 && "index out of range");
    return (&mX)[_index];
  }
  ...
  Vector3f &round();
  Vector3f &lerp(const Vector3f &_start, const Vector3f &_end,
