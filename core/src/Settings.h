...
#define DEFINE_BOOL_SETTING(XX)                                                \
  static bool XX() { return Settings::getInstance()->getBool(#XX); };          \
  static bool set##XX(bool val) {                                              \
    return Settings::getInstance()->setBool(#XX, val);                         \
  };
#define DEFINE_INT_SETTING(XX)                                                 \
  static int XX() { return Settings::getInstance()->getInt(#XX); };            \
  static bool set##XX(int val) {                                               \
    return Settings::getInstance()->setInt(#XX, val);                          \
  };
#define DEFINE_FLOAT_SETTING(XX)                                               \
  static float XX() { return Settings::getInstance()->getFloat(#XX); };        \
  static bool set##XX(float val) {                                             \
    return Settings::getInstance()->setFloat(#XX, val);                        \
  };
#define DEFINE_STRING_SETTING(XX)                                              \
  static std::string XX() { return Settings::getInstance()->getString(#XX); }; \
  static bool set##XX(const std::string &val) {                                \
    return Settings::getInstance()->setString(#XX, val);                       \
  };
...
#define UPDATE_STATIC_BOOL_SETTING(XX)                                         \
  if (name == #XX)                                                             \
    _##XX = getBool(#XX);
#define UPDATE_STATIC_BOOL_SETTING_EX(NAME, XX)                                \
  if (name == NAME)                                                            \
    _##XX = getBool(NAME);

#define IMPLEMENT_STATIC_INT_SETTING(XX, VAL) int Settings::_##XX = VAL;
#define UPDATE_STATIC_INT_SETTING(XX)                                          \
  if (name == #XX)                                                             \
    _##XX = getInt(#XX);
...
public:
  ...
  // You will get a warning if you try a get on a key that is not already
  // present.
  ...
  // Cached settings using static fields. They must be implemented using
  // IMPLEMENT_STATIC_xx_SETTING & updated with UPDATE_STATIC_xxx_SETTING
