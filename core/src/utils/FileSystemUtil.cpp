...
stringList getDirContent(const std::string &_path, const bool _recursive,
  ...
  if (isDirectory(path)) {
    ...
    HANDLE hFind =
        FindFirstFileExW(Utils::String::convertToWideString(wildcard).c_str(),
                         FINDEX_INFO_LEVELS::FindExInfoBasic, &findData,
                         FINDEX_SEARCH_OPS::FindExSearchNameMatch, NULL,
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
      ...
      do {
        std::string name =
            Utils::String::convertFromWideString(findData.cFileName);
        ...
        if (!includeHidden && (findData.dwFileAttributes &
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
      ...
      do {
        ...
        FileInfo fi;
                  ...
                  Utils::String::convertFromWideString(findData.cFileName);
        fi.hidden = (findData.dwFileAttributes & FILE_ATTRIBUTE_HIDDEN) ==
                    FILE_ATTRIBUTE_HIDDEN;
        fi.directory = (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ==
                       FILE_ATTRIBUTE_DIRECTORY;
...
std::string getGenericPath(const std::string &_path) {
  ...
  // remove trailing '/'
  while (path.length() &&
         ((offset = path.find_last_of('/')) == (path.length() - 1)))
    path.erase(offset, 1);
...
#else  // _WIN32
       // insert a backslash before most characters that would mess up a bash
       // path
...
std::string changeExtension(const std::string &_path,
...
std::string createRelativePath(const std::string &_path,
  ...
  if (_allowHome) {
    ...
    if (from_dirs.size() == 0 || to_dirs.size() == 0 ||
        from_dirs[0] != to_dirs[0])
      return path;
...
std::string removeCommonPath(const std::string &_path,
  ...
  // std::string common = isDirectory(_common) ? getGenericPath(_common) :
  // getParent(_common);
  ...
  if (path.find(_common) == 0 && path != _common) {
    _contains = true;
    int trailingSlash =
        _common.find_last_of('/') == (_common.length() - 1) ? 0 : 1;
...
std::string resolveSymlink(const std::string &_path) {
  ...
  if (hFile != INVALID_HANDLE_VALUE) {
    resolved.resize(
        GetFinalPathNameByHandle(hFile, nullptr, 0, FILE_NAME_NORMALIZED) + 1);
...
bool removeDirectory(const std::string &_path) {
  ...
  return RemoveDirectoryW(
      Utils::String::convertToWideString(getPreferredPath(_path)).c_str());
#else
...
bool removeFile(const std::string &_path) {
  ...
  if (!DeleteFileW(Utils::String::convertToWideString(_path).c_str())) {
    SetFileAttributesW(Utils::String::convertToWideString(_path).c_str(),
...
#ifdef WIN32
  if (::CreateDirectoryW(Utils::String::convertToWideString(_path).c_str(),
                         nullptr))
    return true;
#endif
...
bool exists(const std::string &_path) {
  ...
  if (!FileCache::isEnabled())
    return _waccess_s(Utils::String::convertToWideString(_path).c_str(), 0) ==
           0;

  DWORD dwAttr =
      GetFileAttributesW(Utils::String::convertToWideString(_path).c_str());
...
bool isRegularFile(const std::string &_path) {
  ...
  if (FileCache::fromStat64(path, &info) !=
      0) // if(stat64(path.c_str(), &info) != 0)
    return false;
...
bool isDirectory(const std::string &_path) {
  ...
  if (S_ISLNK(info.st_mode)) {
    if (FileCache::fromStat64(resolveSymlink(path), &info) !=
        0) // if(stat64(path.c_str(), &info) != 0)
      return false;
  }
...
bool isHidden(const std::string &_path) {
  ...
  // filenames starting with . are hidden in linux, we do this check for windows
  // as well
...
std::string combine(const std::string &_path, const std::string &filename) {
  ...
  if (!Utils::String::endsWith(gp, "/") && !Utils::String::endsWith(gp, "\\"))
    if (!Utils::String::startsWith(filename, "/") &&
        !Utils::String::startsWith(filename, "\\"))
      gp += "/";
...
static void skipUtf8Bom(std::ifstream &file) {
  ...
  char bom[3];
  if (file.read(bom, 3) && bom[0] == '\xEF' && bom[1] == '\xBB' &&
      bom[2] == '\xBF')
    return;
...
#if defined(_WIN32)
  FILE *file =
      _wfopen(Utils::String::convertToWideString(fileName).c_str(), L"wb");
#else
...
#if WIN32
  return MoveFileW(Utils::String::convertToWideString(path).c_str(),
                   Utils::String::convertToWideString(dst).c_str());
#else
...
#if defined(_WIN32)
  FILE *source =
      _wfopen(Utils::String::convertToWideString(path).c_str(), L"rb");
#else
...
#if defined(_WIN32)
  FILE *dest =
      _wfopen(Utils::String::convertToWideString(pathD).c_str(), L"wb");
#else
...
std::string getTempPath() {
  ...
  if (path.empty()) {
#ifdef WIN32
    // Set tmp files to local drive : usually faster since it's generally a SSD
    // Drive
    WCHAR lpTempPathBuffer[MAX_PATH];
    lpTempPathBuffer[0] = 0;
    DWORD dwRetVal = ::GetTempPathW(MAX_PATH, lpTempPathBuffer);
    if (dwRetVal > 0 && dwRetVal <= MAX_PATH)
      path = Utils::FileSystem::getGenericPath(
                 Utils::String::convertFromWideString(lpTempPathBuffer)) +
             "/emulationstation.tmp";
    else
#endif
      path = Utils::FileSystem::getGenericPath(
          Paths::getUserEmulationStationPath() + "/tmp");
  }
...
std::string getPdfTempPath() {
  ...
  if (pdfpath.empty()) {
#ifdef WIN32
    // Extract PDFs to local drive : usually faster since it's generally a SSD
    // Drive
    WCHAR lpTempPathBuffer[MAX_PATH];
    lpTempPathBuffer[0] = 0;
    DWORD dwRetVal = ::GetTempPathW(MAX_PATH, lpTempPathBuffer);
    if (dwRetVal > 0 && dwRetVal <= MAX_PATH)
      pdfpath = Utils::FileSystem::getGenericPath(
                    Utils::String::convertFromWideString(lpTempPathBuffer)) +
                "/pdftmp";
    else
#endif
      pdfpath = Utils::FileSystem::getGenericPath(
          Paths::getUserEmulationStationPath() + "/pdftmp");
  }
...
#if defined(_WIN32)
  FILE *file =
      _wfopen(Utils::String::convertToWideString(filename).c_str(), L"rb");
#else
...
static std::set<std::string> _imageExtensions = {".jpg", ".png", ".jpeg",
                                                 ".gif", ".webp"};
static std::set<std::string> _videoExtensions = {".mp4", ".avi", ".mkv",
                                                 ".webm"};
static std::set<std::string> _audioExtensions = {
    ".mp3", ".wav", ".ogg", ".flac", ".mod", ".xm",  ".stm",
    ".s3m", ".far", ".it",  ".669",  ".mtm", ".opus"};

bool isSVG(const std::string &_path) {
  return Utils::String::toLower(Utils::FileSystem::getExtension(_path)) ==
         ".svg";
}

bool isImage(const std::string &_path) {
  return _imageExtensions.find(
             Utils::String::toLower(Utils::FileSystem::getExtension(_path))) !=
         _imageExtensions.cend();
}

bool isVideo(const std::string &_path) {
  return _videoExtensions.find(
             Utils::String::toLower(Utils::FileSystem::getExtension(_path))) !=
         _videoExtensions.cend();
}

bool isAudio(const std::string &_path) {
  return _audioExtensions.find(
             Utils::String::toLower(Utils::FileSystem::getExtension(_path))) !=
         _audioExtensions.cend();
}

#ifdef WIN32
void splitCommand(std::string cmd, std::string *executable,
  ...
  auto doWork = [&](std::string *path) {
    if (trySaveStates) {
      ...
      for (auto file : Utils::FileSystem::getDirContent(
               Utils::FileSystem::combine(Paths::getSavesPath(), systemName)))
        Utils::FileSystem::exists(file);
    }

    for (auto file : Utils::FileSystem::getDirContent(
             Utils::FileSystem::combine(*path, "/images")))
      Utils::FileSystem::exists(file);

    for (auto file : Utils::FileSystem::getDirContent(
             Utils::FileSystem::combine(*path, "/videos")))
      Utils::FileSystem::exists(file);

    for (auto file : Utils::FileSystem::getDirContent(
             Utils::FileSystem::combine(*path, "/manuals")))
      Utils::FileSystem::exists(file);
