...
int ProcessStartInfo::run() const {
  ...
  lpExecInfo.nShow =
      showWindow ? SW_SHOW
                 : SW_HIDE; // show command prompt with normal window size
  lpExecInfo.hInstApp =
      (HINSTANCE)SE_ERR_DDEFAIL;           // WINSHELLAPI BOOL WINAPI result;
  lpExecInfo.lpParameters = wargs.c_str(); //  file name as an argument

  std::wstring wpath;

  // Don't set directory for relative paths
  if (!Utils::String::startsWith(exe, ".") &&
      !Utils::String::startsWith(exe, "/") &&
      !Utils::String::startsWith(exe, "\\")) {
    wpath = Utils::String::convertToWideString(
  ...
  std::string cmdOutput =
      " 2> " + Utils::FileSystem::combine(Paths::getLogPath(), stderrFilename) +
      " | head -300 > " +
      Utils::FileSystem::combine(Paths::getLogPath(), stdoutFilename);
  if (!Log::enabled())
  ...
  if (ret == 0) {
    ret = fork();
    if (ret == 0) {
      execl("/bin/sh", "sh", "-c", (cmd_utf8 + cmdOutput).c_str(),
            (char *)NULL);
...
bool isFastShutdown() {
  return quitMode == QuitMode::FAST_REBOOT ||
         quitMode == QuitMode::FAST_SHUTDOWN;
}
  ...
  for (ifa = ifAddrStruct; ifa != NULL; ifa = ifa->ifa_next) {
    ...
    if (ifa->ifa_addr->sa_family == AF_INET) {
      ...
      if (ifName.find("eth") != std::string::npos ||
          ...
          ifName.find("mlan") != std::string::npos ||
          ifName.find("en") != std::string::npos ||
          ifName.find("wl") != std::string::npos ||
          ifName.find("p2p") != std::string::npos) {
...
BatteryInformation queryBatteryInformation() {
  ...
  if (batteryStatusPath.empty()) {
    ...
    for (auto file : files) {
      if ((Utils::String::toLower(file).find("/bat") != std::string::npos) &&
          (batteryRootPath.empty()))
        batteryRootPath = file;

      if ((Utils::String::toLower(file).find("fuel") != std::string::npos) &&
          (fuelgaugeRootPath.empty()))
        fuelgaugeRootPath = file;

      if ((Utils::String::toLower(file).find("charger") != std::string::npos) &&
          (chargerRootPath.empty()))
    ...
    if (batteryRootPath.empty()) {
      if (!fuelgaugeRootPath.empty()) {
        ...
        batteryCapacityPath = ".";
        // If there's a fuel gauge without "charge_now" or "charge_full"
        // property, don't poll it
        if ((!Utils::FileSystem::exists(batteryCurrChargePath)) ||
            (!Utils::FileSystem::exists(batteryMaxChargePath))) {
  ...
  if ((batteryStatusPath.length() <= 1) &&
      (batteryCurrChargePath.length() <= 1)) {
    ret.hasBattery = false;
    ret.isCharging = false;
    ret.level = 0;
  } else {
    ...
    chargerStatus = Utils::String::replace(
        Utils::FileSystem::readAllText(batteryStatusPath), "\n", "");
    ret.isCharging =
        ((chargerStatus != "Not charging") && (chargerStatus != "Discharging"));
    ...
    if (batteryCapacityPath.length() <= 1) {
      if ((!batteryCurrChargePath.empty()) && (!batteryMaxChargePath.empty())) {
        float now = std::stof(
            Utils::FileSystem::readAllText(batteryCurrChargePath).c_str());
        float full = std::stof(
            Utils::FileSystem::readAllText(batteryMaxChargePath).c_str());
...
#if WIN32
static bool _getWindowsVersion(WORD &major, WORD &minor, WORD &build,
                               WORD &revision) {
  // Use product version in kernel32.dll to have real Windows version as we
  // don't have a compatibility manifest (GetVersion is limited to 6.2)
...
#else
  std::string arch =
      Utils::FileSystem::readAllText("/usr/share/batocera/batocera.arch");
  if (!arch.empty())
    return arch;
