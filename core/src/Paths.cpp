...
Paths::Paths() {
  ...
  mUserEmulationStationPath =
      Utils::FileSystem::getCanonicalPath(getHomePath() + "/.emulationstation");
  ...
  loadCustomConfiguration(
      false); // Try to detect alternate paths ( Decorations, Shaders... )
              // without loading overrides
  ...
  /* EmuElec sample locations.
          ...
          mUserEmulationStationPath =
  Utils::FileSystem::getCanonicalPath(Utils::FileSystem::getHomePath() +
  "/.emulationstation"); mLogPath = "/storage/.config/emuelec/logs"; mThemesPath
  = mEmulationStationPath + "/themes"; mUserThemesPath = "/emuelec/themes";
  ...
  */
  loadCustomConfiguration(
      true); // Load paths overrides from emulationstation.ini file
}
...
void Paths::loadCustomConfiguration(bool overridesOnly) {
  ...
  std::string path =
      mEmulationStationPath + std::string("/") + SETTINGS_FILENAME;
  if (!Utils::FileSystem::exists(path))
    path = mUserEmulationStationPath + std::string("/") + SETTINGS_FILENAME;
  if (!Utils::FileSystem::exists(path))
    path = Utils::FileSystem::getParent(mUserEmulationStationPath) +
  ...
  if (!overridesOnly) {
    for (auto var : folders) {
      ...
      if (variable == "kodi") {
        ...
        else if (Utils::FileSystem::exists(
                     "C:\\Program Files (x86)\\Kodi\\kodi.exe"))
          ret[variable] = "C:\\Program Files (x86)\\Kodi\\kodi.exe";
        else if (Utils::FileSystem::exists(Utils::FileSystem::combine(
                     Utils::FileSystem::getParent(relativeTo),
                     "kodi\\kodi.exe")))
          ret[variable] = Utils::FileSystem::combine(
              Utils::FileSystem::getParent(relativeTo), "kodi\\kodi.exe");
      ...
      if (Utils::String::startsWith(variable, "system.")) {
        ...
        auto dir = Utils::FileSystem::getCanonicalPath(
            mUserEmulationStationPath + "/" + name);
        if (Utils::FileSystem::isDirectory(dir))
          ret[variable] = dir;
        else {
          auto dir = Utils::FileSystem::getCanonicalPath(relativeTo +
                                                         "/../system/" + name);
          if (Utils::FileSystem::isDirectory(dir))
            ret[variable] = dir;
        }
        ...
        auto dir =
            Utils::FileSystem::getCanonicalPath(relativeTo + "/../" + variable);
        if (Utils::FileSystem::isDirectory(dir))
          ret[variable] = dir;
        else {
          auto dir = Utils::FileSystem::getCanonicalPath(
              relativeTo + "/../system/" + variable);
          if (Utils::FileSystem::isDirectory(dir))
            ret[variable] = dir;
        }
      }
    ...
    for (auto var : files) {
      auto file =
          Utils::FileSystem::resolveRelativePath(var.first, relativeTo, true);
      if (Utils::FileSystem::exists(file) &&
          !Utils::FileSystem::isDirectory(file))
        ret[var.first] = file;
    }
  } else {
    std::string line;
    ...
    if (systemConf && systemConf.is_open()) {
      while (std::getline(systemConf, line)) {
        ...
        if (!key.empty() && !value.empty()) {
          auto dir =
              Utils::FileSystem::resolveRelativePath(value, relativeTo, true);
          if (Utils::FileSystem::isDirectory(dir))
            ret[key] = dir;
        }
      }
...
std::string &Paths::getHomePath() {
...
#ifdef WIN32
  // Is it a portable installation ? Check if ".emulationstation/es_systems.cfg"
  // exists in the exe's path
  if (!homePath.length()) {
    std::string portableCfg =
        getExePath() + "/.emulationstation/es_systems.cfg";
  ...
  // On Windows,  "HOME" is not a system variable but a user's environment
  // variable that can be defined by users in batch files. If defined : The
  // environment variable has priority over all
  char *envHome = getenv("HOME");
  if (envHome)
    homePath = Utils::FileSystem::getGenericPath(envHome);

#ifdef WIN32
  // On Windows, getenv("HOME") is not the system's user path but a user
  // environment variable. Instead we get the home user's path using
  // %HOMEDRIVE%/%HOMEPATH% which are system variables.
  if (!homePath.length()) {
    char *envHomeDrive = getenv("HOMEDRIVE");
    char *envHomePath = getenv("HOMEPATH");
    if (envHomeDrive && envHomePath)
      homePath = Utils::FileSystem::getGenericPath(std::string(envHomeDrive) +
                                                   "/" + envHomePath);
  }
#endif // _WIN32
...
std::string &Paths::getExePath() { return exePath; }
...
std::string Paths::findEmulationStationFile(const std::string &fileName) {
  std::string localVersionFile =
      Paths::getEmulationStationPath() + "/" + fileName;
