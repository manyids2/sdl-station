...
ShaderBatch *ShaderBatch::getShaderBatch(const char *shaderFile) {
  ...
  std::string fullPath =
      ResourceManager::getInstance()->getResourcePath(shaderFile);

  ShaderBatch *ret = new ShaderBatch();

  std::string ext =
      Utils::String::toLower(Utils::FileSystem::getExtension(fullPath));
  if (ext == ".glslp") {
    ...
    if (systemConf && systemConf.is_open()) {
      while (std::getline(systemConf, line)) {
        int idx = line.find("=");
        if (idx == std::string::npos || line.find("#") == 0 ||
            line.find(";") == 0)
          continue;

        std::string key = Utils::String::trim(line.substr(0, idx));
        std::string value = Utils::String::trim(
            Utils::String::replace(line.substr(idx + 1), "\"", ""));
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!Utils::String::startsWith(relative, ":") &&
          !Utils::String::startsWith(relative, "/") &&
          !Utils::String::startsWith(relative, "."))
        relative = "./" + relative;

      std::string full =
          Utils::FileSystem::resolveRelativePath(relative, path, true);
...
static void setupDefaultShaders() {
  ...
  std::string shaders = Utils::String::trim(
  ...
  std::string vertexSourceNoTexture = SHADER_VERSION_STRING +
                                      R"=====(
  ...
  std::string fragmentSourceNoTexture = SHADER_VERSION_STRING +
                                        R"=====(
			#ifdef GL_ES
  ...
  auto vertexShaderNoTexture =
      Shader::createShader(GL_VERTEX_SHADER, vertexSourceNoTexture);
  auto fragmentShaderColorNoTexture =
      Shader::createShader(GL_FRAGMENT_SHADER, fragmentSourceNoTexture);

  shaderProgramColorNoTexture.createShaderProgram(vertexShaderNoTexture,
                                                  fragmentShaderColorNoTexture);

  // vertex shader (texture)
  std::string vertexSourceTexture = SHADER_VERSION_STRING +
                                    R"=====(
			uniform   mat4 MVPMatrix;
			attribute vec2 VertexCoord;
  ...
  std::string fragmentSourceTexture = SHADER_VERSION_STRING +
                                      R"=====(
			#ifdef GL_ES
			precision mediump float;
  ...
  // Compile each shader, link them to make a full program
  auto vertexShaderTexture =
      Shader::createShader(GL_VERTEX_SHADER, vertexSourceTexture);
  auto fragmentShaderColorTexture =
      Shader::createShader(GL_FRAGMENT_SHADER, fragmentSourceTexture);
  shaderProgramColorTexture.createShaderProgram(vertexShaderTexture,
                                                fragmentShaderColorTexture);

  // fragment shader (alpha texture)
  std::string fragmentSourceAlpha = SHADER_VERSION_STRING +
                                    R"=====(
			#ifdef GL_ES
			precision mediump float;
			precision mediump sampler2D;
  ...
  auto vertexShaderAlpha =
      Shader::createShader(GL_VERTEX_SHADER, vertexSourceTexture);
  auto fragmentShaderAlpha =
      Shader::createShader(GL_FRAGMENT_SHADER, fragmentSourceAlpha);

  shaderProgramAlpha.createShaderProgram(vertexShaderAlpha,
                                         fragmentShaderAlpha);
...
static int getAvailableVideoMemory() {
  /*
  const std::string extensions = glGetString(GL_EXTENSIONS) ? (const
  char*)glGetString(GL_EXTENSIONS) : ""; if
  (extensions.find("GL_NVX_gpu_memory_info") != std::string::npos)
  {
          GLint totalMemoryKb = 0;
          glGetIntegerv(GL_GPU_MEMORY_INFO_TOTAL_AVAILABLE_MEMORY_NVX,
  &totalMemoryKb); if (totalMemoryKb != 0) return totalMemoryKb / 1024;
  }
  */
  float total = 0;
  ...
  while (true) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, sz, sz, 0, GL_RGBA,
...
GLES20Renderer::getDriverInformation() {
  std::vector<std::pair<std::string, std::string>> info;

  info.push_back(
      std::pair<std::string, std::string>("GRAPHICS API", getDriverName()));
  ...
  info.push_back(std::pair<std::string, std::string>("USED VRAM",
  std::to_string(videoMem) + " MB"));

  videoMem = getAvailableVideoMemory();
  info.push_back(std::pair<std::string, std::string>("FREE VRAM",
  std::to_string(videoMem) + " MB"));
  */
  return info;
}
...
unsigned int
GLES20Renderer::createTexture(const Texture::Type _type, const bool _linear,
...
void GLES20Renderer::updateTexture(const unsigned int _texture,
...
void GLES20Renderer::drawLines(const Vertex *_vertices,
                               const unsigned int _numVertices,
                               const Blend::Factor _srcBlendFactor,
                               const Blend::Factor _dstBlendFactor) {
  // Pass buffer data
  GL_CHECK_ERROR(glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * _numVertices,
...
//////////////////////////////////////////////////////////////////////////
void GLES20Renderer::drawSolidRectangle(const float _x, const float _y,
                                        const float _w, const float _h,
                                        const unsigned int _fillColor,
                                        const unsigned int _borderColor,
                                        float borderWidth, float cornerRadius) {
  if (cornerRadius == 0.0f) {
    if (_fillColor != 0)
      drawRect(_x + borderWidth, _y + borderWidth,
               _w - borderWidth - borderWidth, _h - borderWidth - borderWidth,
               _fillColor);

    if (_borderColor != 0 && borderWidth > 0) {
      drawRect(_x, _y, _w, borderWidth, _borderColor);
      drawRect(_x + _w - borderWidth, _y + borderWidth, borderWidth,
               _h - borderWidth, _borderColor);
      drawRect(_x, _y + _h - borderWidth, _w - borderWidth, borderWidth,
               _borderColor);
      drawRect(_x, _y + borderWidth, borderWidth,
               _h - borderWidth - borderWidth, _borderColor);
    }
    return;
  }
  ...
  GL_CHECK_ERROR(glEnable(GL_BLEND));
  GL_CHECK_ERROR(glBlendFunc(convertBlendFactor(Blend::SRC_ALPHA),
                             convertBlendFactor(Blend::ONE_MINUS_SRC_ALPHA)));

  auto inner = createRoundRect(
      _x + borderWidth, _y + borderWidth, _w - borderWidth - borderWidth,
      _h - borderWidth - borderWidth, cornerRadius, _fillColor);

  if ((_fillColor) & 0xFF) {
    GL_CHECK_ERROR(glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * inner.size(),
                                inner.data(), GL_DYNAMIC_DRAW));
  ...
  if ((_borderColor) & 0xFF && borderWidth > 0) {
    ...
    GL_CHECK_ERROR(glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * outer.size(),
                                outer.data(), GL_DYNAMIC_DRAW));
...
void GLES20Renderer::drawTriangleStrips(const Vertex *_vertices,
                                        const unsigned int _numVertices,
                                        const Blend::Factor _srcBlendFactor,
                                        const Blend::Factor _dstBlendFactor,
  ...
  if (boundTexture != 0) {
    ...
    if (it != _textures.cend() && it->second != nullptr &&
        it->second->type == GL_ALPHA)
      useProgram(&shaderProgramAlpha);
    else {
      ShaderProgram *shader = &shaderProgramColorTexture;

      if (_vertices->customShader != nullptr &&
          !_vertices->customShader->path.empty()) {
        ShaderProgram *customShader =
            getShaderProgram(_vertices->customShader->path.c_str());
        if (customShader != nullptr)
          shader = customShader;
      }
      ...
      if (shader->supportsTextureSize() && it != _textures.cend() &&
          it->second != nullptr) {
      ...
      if (_numVertices > 0) {
        ...
        // Inverted rendering
        if (_vertices[_numVertices - 1].tex.y() == 1 &&
            _vertices[0].tex.y() == 0)
          vec.y() = -vec.y();
      ...
      if (_vertices->customShader != nullptr &&
          !_vertices->customShader->path.empty())
        shader->setCustomUniformsParameters(
            _vertices->customShader->parameters);
    }
  } else
    useProgram(&shaderProgramColorNoTexture);
...
void GLES20Renderer::setViewport(const Rect &_viewport) {
  ...
  GL_CHECK_ERROR(glViewport(_viewport.x,
                            getWindowHeight() - _viewport.y - _viewport.h,
...
void GLES20Renderer::setScissor(const Rect &_scissor) {
  if ((_scissor.x == 0) && (_scissor.y == 0) && (_scissor.w == 0) &&
      (_scissor.h == 0)) {
    GL_CHECK_ERROR(glDisable(GL_SCISSOR_TEST));
  } else {
    // glScissor starts at the bottom left of the window
    GL_CHECK_ERROR(glScissor(_scissor.x,
                             getWindowHeight() - _scissor.y - _scissor.h,
...
void GLES20Renderer::setSwapInterval() {
  ...
  if (Settings::getInstance()->getBool("VSync")) {
    ...
    // if vsync is requested, try normal vsync; if that doesn't work, try late
    // swap tearing if that doesn't work, report an error
    if (SDL_GL_SetSwapInterval(1) != 0 && SDL_GL_SetSwapInterval(-1) != 0)
      LOG(LogWarning) << "Tried to enable vsync, but failed! ("
                      << SDL_GetError() << ")";
  } else
...
void GLES20Renderer::drawTriangleFan(const Vertex *_vertices,
                                     const unsigned int _numVertices,
                                     const Blend::Factor _srcBlendFactor,
                                     const Blend::Factor _dstBlendFactor) {
  // Pass buffer data
...
void GLES20Renderer::setStencil(const Vertex *_vertices,
                                const unsigned int _numVertices) {
  useProgram(&shaderProgramColorNoTexture);
  ...
  glEnable(GL_BLEND);
  glBlendFunc(convertBlendFactor(Blend::SRC_ALPHA),
              convertBlendFactor(Blend::ONE_MINUS_SRC_ALPHA));
...
void GLES20Renderer::disableStencil() { glDisable(GL_STENCIL_TEST); }
...
size_t GLES20Renderer::getTotalMemUsage() {
  ...
  for (auto tex : _textures) {
    if (tex.first != 0 && tex.second) {
      size_t size = tex.second->size.x() * tex.second->size.y() *
                    (tex.second->type == GL_ALPHA ? 1 : 4);
      total += size;
    }
  }
...
void GLES20Renderer::postProcessShader(
    const std::string &path, const float _x, const float _y, const float _w,
    const float _h, const std::map<std::string, std::string> &parameters,
    unsigned int *data) {
#if OPENGL_EXTENSIONS
  ...
  if (nTextureID > 0) {
    ...
    if (shaderBatch->size() == 1 && data == nullptr) {
      ...
      vertices[2] = {
          {(float)0 + w, (float)height - h}, {1.0f, 1.0f}, 0xFFFFFFFF};
    ...
    GL_CHECK_ERROR(glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * 4, &vertices,
                                GL_DYNAMIC_DRAW));
    ...
    for (int i = 0; i < shaderBatch->size(); i++) {
      ...
      if (i == 0) {
        ...
        GL_CHECK_ERROR(glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER,
                                              GL_COLOR_ATTACHMENT0,
                                              GL_TEXTURE_2D, nTextureID, 0));

        if (getScreenRotate() == 2)
          GL_CHECK_ERROR(glBlitFramebuffer(x, y, x + w, y + h, 0, 0, tw, th,
                                           GL_COLOR_BUFFER_BIT, GL_LINEAR));
        else
          GL_CHECK_ERROR(glBlitFramebuffer(x, height - y - h, x + w, height - y,
                                           0, 0, tw, th, GL_COLOR_BUFFER_BIT,
                                           GL_LINEAR));
        ...
        if (i == shaderBatch->size() - 1 && data == nullptr) {
          ...
          vertices[2] = {{(float)x + w, (float)y}, {1.0f, 1.0f}, 0xFFFFFFFF};
          vertices[3] = {
              {(float)x + w, (float)y + h}, {1.0f, 0.0f}, 0xFFFFFFFF};
          ...
          if (getScreenRotate() == 2) {
            ...
            vertices[2] = {{(float)x + w, (float)y}, {0.0f, 0.0f}, 0xFFFFFFFF};
            vertices[3] = {
                {(float)x + w, (float)y + h}, {0.0f, 1.0f}, 0xFFFFFFFF};
          }

          for (int i = 0; i < 4; ++i)
            vertices[i].pos.round();

          GL_CHECK_ERROR(glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * 4,
                                      &vertices, GL_DYNAMIC_DRAW));

          GL_CHECK_ERROR(glBindFramebuffer(GL_FRAMEBUFFER, 0));
        } else
          GL_CHECK_ERROR(glBindFramebuffer(
              GL_DRAW_FRAMEBUFFER, i % 2 == 1 ? mFrameBuffer : nFrameBuffer2));
      }
