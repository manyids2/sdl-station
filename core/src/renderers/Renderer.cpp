...
static Vector2i sdlWindowPosition =
...
Rect getScreenRect(const Transform4x4f &transform, const Vector3f &size,
                   bool viewPort) {
  return getScreenRect(transform, Vector2f(size.x(), size.y()), viewPort);
}

Rect getScreenRect(const Transform4x4f &transform, const Vector2f &size,
                   bool viewPort) {
  auto rc = Rect((int)Math::round(transform.translation().x()),
...
static void setIcon() {
  ...
  ResourceData resData =
      ResourceManager::getInstance()->getFileData(":/window_icon_256.png");
  unsigned char *rawData = ImageIO::loadFromMemoryRGBA32(
      resData.ptr.get(), resData.length, width, height);
  ...
  if (rawData != nullptr) {
    ...
    SDL_Surface *logoSurface =
        SDL_CreateRGBSurfaceFrom((void *)rawData, (int)width, (int)height, 32,
                                 (int)(width * 4), rmask, gmask, bmask, amask);
...
static bool createWindow() {
  ...
  if (monitorId >= 0 &&
    ...
    if (displays > monitorId) {
      ...
      if (!Settings::getInstance()->getBool("Windowed") &&
          !Settings::getInstance()->getBool("WindowWidth")) {
      ...
      if (Settings::getInstance()->getBool("Windowed") &&
        ...
        if (windowWidth != rc.w || windowHeight != rc.h) {
          sdlWindowPosition = Vector2i(rc.x + (rc.w - windowWidth) / 2,
  ...
  unsigned int windowFlags =
      (Settings::getInstance()->getBool("Windowed")
           ? 0
           : (Settings::getInstance()->getBool("FullscreenBorderless")
...
void deinit() { destroyWindow(); } // deinit

void pushClipRect(const Vector2i &_pos, const Vector2i &_size) {
  pushClipRect(_pos.x(), _pos.y(), _size.x(), _size.y());
}

void pushClipRect(Rect rect) { pushClipRect(rect.x, rect.y, rect.w, rect.h); }
...
void pushClipRect(int x, int y, int w, int h) {
  ...
  case 3: {
    box = Rect(screenOffsetY + box.y,
               windowHeight - screenOffsetX - box.x - box.w, box.h, box.w);
  } break;
  }
...
void drawRect(const float _x, const float _y, const float _w, const float _h,
              const unsigned int _color, const Blend::Factor _srcBlendFactor,
              const Blend::Factor _dstBlendFactor) {
  drawRect(_x, _y, _w, _h, _color, _color, true, _srcBlendFactor,
           _dstBlendFactor);
} // drawRect

void drawRect(const float _x, const float _y, const float _w, const float _h,
              const unsigned int _color, const unsigned int _colorEnd,
              bool horizontalGradient, const Blend::Factor _srcBlendFactor,
              const Blend::Factor _dstBlendFactor) {
  ...
  vertices[1] = {
      {_x, _y + _h}, {0.0f, 0.0f}, horizontalGradient ? colorEnd : color};
  vertices[2] = {
      {_x + _w, _y}, {0.0f, 0.0f}, horizontalGradient ? color : colorEnd};
...
std::map<std::string, float> ratios = {
    {"4/3", 4.0f / 3.0f},     {"16/9", 16.0f / 9.0f},
    {"16/10", 16.0f / 10.0f}, {"16/15", 16.0f / 15.0f},
    {"21/9", 21.0f / 9.0f},   {"1/1", 1 / 1},
    {"2/1", 2.0f / 1.0f},     {"3/2", 3.0f / 2.0f},
    {"3/4", 3.0f / 4.0f},     {"4/1", 4.0f / 1.0f},
    {"9/16", 9.0f / 16.0f},   {"5/4", 5.0f / 4.0f},
    {"6/5", 6.0f / 5.0f},     {"7/9", 7.0f / 9.0f},
    {"8/3", 8.0f / 3.0f},     {"8/7", 8.0f / 7.0f},
    {"19/12", 19.0f / 12.0f}, {"19/14", 19.0f / 14.0f},
...
bool isVisibleOnScreen(float x, float y, float w, float h) {
  static Rect screen =
      Rect(0, 0, Renderer::getScreenWidth(), Renderer::getScreenHeight());
...
unsigned int mixColors(unsigned int first, unsigned int second, float percent) {
  ...
  unsigned char alpha =
      (unsigned char)(alpha0 * (1.0 - percent) + alpha1 * percent);
  unsigned char blue =
      (unsigned char)(blue0 * (1.0 - percent) + blue1 * percent);
  unsigned char green =
      (unsigned char)(green0 * (1.0 - percent) + green1 * percent);
...
static void addRoundCorner(float x, float y, double sa, double arc, float r,
                           unsigned int color, float pieces,
...
std::vector<Vertex> createRoundRect(float x, float y, float width, float height,
  ...
  std::vector<Vertex> vertex;
  addRoundCorner(x, y + radius, 3.0f * ES_PI / 2.0f, ES_PI / 2.0f, radius,
                 finalColor, pieces, vertex);
  addRoundCorner(x + width - radius, y, 0.0, ES_PI / 2.0f, radius, finalColor,
                 pieces, vertex);
  addRoundCorner(x + width, y + height - radius, ES_PI / 2.0f, ES_PI / 2.0f,
                 radius, finalColor, pieces, vertex);
  addRoundCorner(x + radius, y + height, ES_PI, ES_PI / 2.0f, radius,
                 finalColor, pieces, vertex);
  return vertex;
}

void drawRoundRect(float x, float y, float width, float height, float radius,
                   unsigned int color, const Blend::Factor _srcBlendFactor,
                   const Blend::Factor _dstBlendFactor) {
  bindTexture(0);

  std::vector<Vertex> vertex =
      createRoundRect(x, y, width, height, radius, color);
  drawTriangleFan(vertex.data(), vertex.size(), _srcBlendFactor,
                  _dstBlendFactor);
}

void enableRoundCornerStencil(float x, float y, float width, float height,
                              float radius) {
...
static IRenderer *createRenderer() {
  IRenderer *instance =
      getRendererFromName(Settings::getInstance()->getString("Renderer"));
  if (instance == nullptr) {
#ifdef RENDERER_GLES_20
...
//////////////////////////////////////////////////////////////////////////
std::string getDriverName() { return Instance()->getDriverName(); }

std::vector<std::pair<std::string, std::string>> getDriverInformation() {
  return Instance()->getDriverInformation();
}

unsigned int getWindowFlags() { return Instance()->getWindowFlags(); }

void setupWindow() { return Instance()->setupWindow(); }

void createContext() { Instance()->createContext(); }

void resetCache() { Instance()->resetCache(); }

void destroyContext() { Instance()->destroyContext(); }

unsigned int createTexture(const Texture::Type _type, const bool _linear,
                           const bool _repeat, const unsigned int _width,
                           const unsigned int _height, void *_data) {
  return Instance()->createTexture(_type, _linear, _repeat, _width, _height,
                                   _data);
}
...
void updateTexture(const unsigned int _texture, const Texture::Type _type,
...
void drawLines(const Vertex *_vertices, const unsigned int _numVertices,
               const Blend::Factor _srcBlendFactor,
               const Blend::Factor _dstBlendFactor) {
  Instance()->drawLines(_vertices, _numVertices, _srcBlendFactor,
                        _dstBlendFactor);
}
...
void drawTriangleStrips(const Vertex *_vertices,
  ...
  Instance()->drawTriangleStrips(_vertices, _numVertices, _srcBlendFactor,
                                 _dstBlendFactor, verticesChanged);
}

void drawSolidRectangle(const float _x, const float _y, const float _w,
                        const float _h, const unsigned int _fillColor,
                        const unsigned int _borderColor, float borderWidth,
                        float cornerRadius) {
  Instance()->drawSolidRectangle(_x, _y, _w, _h, _fillColor, _borderColor,
                                 borderWidth, cornerRadius);
}

void drawTriangleFan(const Vertex *_vertices, const unsigned int _numVertices,
                     const Blend::Factor _srcBlendFactor,
                     const Blend::Factor _dstBlendFactor) {
  Instance()->drawTriangleFan(_vertices, _numVertices, _srcBlendFactor,
                              _dstBlendFactor);
}
...
bool supportShaders() { return Instance()->supportShaders(); }

void setProjection(const Transform4x4f &_projection) {
  Instance()->setProjection(_projection);
}

void setMatrix(const Transform4x4f &_matrix) { Instance()->setMatrix(_matrix); }

void blurBehind(const float _x, const float _y, const float _w, const float _h,
...
void postProcessShader(const std::string &path, const float _x, const float _y,
...
void setScissor(const Rect &_scissor) { Instance()->setScissor(_scissor); }

void setStencil(const Vertex *_vertices, const unsigned int _numVertices) {
  Instance()->setStencil(_vertices, _numVertices);
}

void disableStencil() { Instance()->disableStencil(); }

void setSwapInterval() { Instance()->setSwapInterval(); }
...
size_t getTotalMemUsage() { return Instance()->getTotalMemUsage(); }
