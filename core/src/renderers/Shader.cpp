...
bool Shader::compile(GLuint id, const char *source, GLenum type) {
  ...
  if (maxLength > 1) {
    ...
    if (isCompiled == GL_FALSE) {
      ...
      if (strstr(infoLog, "WARNING") || strstr(infoLog, "warning") ||
          strstr(infoLog, "Warning"))
...
ShaderProgram::ShaderProgram()
    : mId(-1), mPositionAttribute(-1), mColorAttribute(-1),
      mTexCoordAttribute(-1), mvpUniform(-1), linkStatus(false),
      mOutputSize(-1), mOutputOffset(-1), mInputSize(-1), mTextureSize(-1),
      mResolution(-1), mSaturation(-1), mCornerRadius(-1), mFrameCount(-1),
      mFrameDirection(-1) {}
...
static std::string appendVersionAndType(const std::string &shaderCode,
...
bool ShaderProgram::loadFromFile(const std::string &path) {
  ...
  const ResourceData &shaderData =
      ResourceManager::getInstance()->getFileData(path);

  std::string shaderCode;
  shaderCode.assign(reinterpret_cast<const char *>(shaderData.ptr.get()),
                    shaderData.length);

  std::string versionString = SHADER_VERSION_STRING;

  Shader vertex = Shader::createShader(
      GL_VERTEX_SHADER, appendVersionAndType(shaderCode, "#define VERTEX"));
  if (!vertex.compileStatus) {
    LOG(LogError) << "Failed to compile GLSL VERTEX shader : " << path;
    return false;
  }

  Shader fragment = Shader::createShader(
      GL_FRAGMENT_SHADER, appendVersionAndType(shaderCode, "#define FRAGMENT"));
...
bool ShaderProgram::createShaderProgram(Shader &vertexShader,
...
static std::set<std::string> builtInUniforms = {
    "MVPMatrix",      "VertexCoord", "positionVertex",  "TexCoord",
    "texCoordVertex", "COLOR",       "colorVertex",     "TextureSize",
    "textureSize",    "OutputSize",  "outputSize",      "OutputOffset",
    "outputOffset",   "InputSize",   "inputSize",       "Resolution",
    "resolution",     "saturation",  "es_cornerRadius", "FrameCount",
    "FrameDirection", "u_tex",       "textureSampler",  "Texture"};
...
void ShaderProgram::findAttribsAndUniforms() {
  ...
  for (int i = 0; i < numUniforms; ++i) {
    ...
    char buffer[256];
    GL_CHECK_ERROR(glGetActiveUniform(mId, i, (GLsizei)sizeof(buffer), &length,
                                      &size, &type, buffer));
...
void ShaderProgram::setMatrix(Transform4x4f &mvpMatrix) {
  if (mvpUniform != -1 && mvpUniform != GL_INVALID_VALUE &&
      mvpUniform != GL_INVALID_OPERATION)
    GL_CHECK_ERROR(
        glUniformMatrix4fv(mvpUniform, 1, GL_FALSE, (float *)&mvpMatrix));
}
...
void ShaderProgram::setResolution() {
  if (mResolution != -1)
    GL_CHECK_ERROR(
        glUniform2f(mResolution, getScreenWidth(), getScreenHeight()));
}
...
void ShaderProgram::setCustomUniformsParameters(
...
void ShaderProgram::setUniformEx(const std::string &name,
  ...
  case GL_BOOL:
    GL_CHECK_ERROR(glUniform1i(
        location, Utils::String::toBoolean(value) ? GL_TRUE : GL_FALSE));
    break;
  case GL_FLOAT_VEC2: {
  ...
  case GL_FLOAT_VEC4: {
    if (value != "0" && value != "1" && value.find(" ") == std::string::npos &&
      ...
      GL_CHECK_ERROR(glUniform4f(location, red / 255.0f, green / 255.0f,
...
void ShaderProgram::select() {
  ...
  if (mPositionAttribute != -1) {
    GL_CHECK_ERROR(glVertexAttribPointer(mPositionAttribute, 2, GL_FLOAT,
  ...
  if (mColorAttribute != -1) {
    GL_CHECK_ERROR(glVertexAttribPointer(mColorAttribute, 4, GL_UNSIGNED_BYTE,
  ...
  if (mTexCoordAttribute != -1) {
    GL_CHECK_ERROR(glVertexAttribPointer(mTexCoordAttribute, 2, GL_FLOAT,
                                         GL_FALSE, sizeof(Vertex),
