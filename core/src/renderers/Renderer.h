...
struct Vertex {
  Vertex()
      : col(0), saturation(1.0f), cornerRadius(0.0f), customShader(nullptr) {}

  Vertex(const Vector2f &_pos, const Vector2f &_tex, const unsigned int _col)
      : pos(_pos), tex(_tex), col(_col), saturation(1.0f), cornerRadius(0.0f),
...
public:
  ...
  virtual void drawLines(
      ...
      const Blend::Factor _dstBlendFactor = Blend::ONE_MINUS_SRC_ALPHA) = 0;
  virtual void drawTriangleStrips(
      const Vertex *_vertices, const unsigned int _numVertices,
      const Blend::Factor _srcBlendFactor = Blend::SRC_ALPHA,
      const Blend::Factor _dstBlendFactor = Blend::ONE_MINUS_SRC_ALPHA,
      bool verticesChanged = true) = 0;
  virtual void drawTriangleFan(
      const Vertex *_vertices, const unsigned int _numVertices,
      const Blend::Factor _srcBlendFactor = Blend::SRC_ALPHA,
      const Blend::Factor _dstBlendFactor = Blend::ONE_MINUS_SRC_ALPHA) = 0;

  virtual void drawSolidRectangle(const float _x, const float _y,
  ...
  virtual void setStencil(const Vertex *_vertices,
                          const unsigned int _numVertices) = 0;
  ...
  virtual void
  postProcessShader(const std::string &path, const float _x, const float _y,
                    const float _w, const float _h,
  ...
  virtual bool shaderSupportsCornerSize(const std::string &shader) {
    return false;
  };
};
...
// API specific
inline static unsigned int convertColor(const unsigned int _color) {
  return ((_color & 0xFF000000) >> 24) | ((_color & 0x00FF0000) >> 8) |
         ((_color & 0x0000FF00) << 8) | ((_color & 0x000000FF) << 24);
}
// convertColor
...
void drawSolidRectangle(const float _x, const float _y, const float _w,
                        ...
                        const unsigned int _borderColor, float borderWidth = 1,
...
inline bool isVisibleOnScreen(const Rect &rect) {
  return isVisibleOnScreen(rect.x, rect.y, rect.w, rect.h);
}
bool isSmallScreen();
unsigned int mixColors(unsigned int first, unsigned int second, float percent);

void drawRoundRect(
    float x, float y, float w, float h, float radius, unsigned int color,
    const Blend::Factor _srcBlendFactor = Blend::SRC_ALPHA,
    const Blend::Factor _dstBlendFactor = Blend::ONE_MINUS_SRC_ALPHA);
void enableRoundCornerStencil(float x, float y, float size_x, float size_y,
                              float radius);
