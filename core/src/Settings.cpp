...
void Settings::updateCachedSetting(const std::string &name) {
  ...
  if (mLoaded)
    settingChanged.invoke(
        [name](ISettingsChangedEvent *c) { c->onSettingChanged(name); });
}

// these values are NOT saved to es_settings.xml
// since they're set through command-line arguments, and not the in-program
// settings menu
...
void Settings::setDefaults() {
...
#if defined(_WIN32) || defined(TINKERBOARD) || defined(X86) ||                 \
    defined(X86_64) || defined(ODROIDN2) || defined(ODROIDC2) ||               \
    defined(ODROIDXU4) || defined(RPI4)
  // Boards > 1Gb RAM
  mIntMap["MaxVRAM"] = 256;
#elif defined(ODROIDGOA) || defined(GAMEFORCE) || defined(RK3326) ||           \
    defined(RPIZERO2) || defined(RPI2) || defined(RPI3) || defined(ROCKPRO64)
  // Boards with 1Gb RAM
  mIntMap["MaxVRAM"] = 128;
#elif defined(_RPI_)
  // Rpi 0, 1
  ...
  mStringMap["CollectionSystemsAuto"] =
      "all,favorites"; // 2players,4players,favorites,recent
...
//              std::map<K, V> &defaultMap, V defaultValue) {
...
//       Paths::getUserEmulationStationPath() + "/es_settings.cfg";
//
...
//   false); saveMap<std::string, int>(config, mIntMap, "int", mDefaultIntMap,
//   0); saveMap<std::string, float>(config, mFloatMap, "float",
//   mDefaultFloatMap, 0);
//
...
//     root = config;
//
//   for (pugi::xml_node node = root.child("bool"); node;
//        node = node.next_sibling("bool"))
//     setBool(node.attribute("name").as_string(),
//             node.attribute("value").as_bool());
//   for (pugi::xml_node node = root.child("int"); node;
//        node = node.next_sibling("int"))
//     setInt(node.attribute("name").as_string(),
//            node.attribute("value").as_int());
//   for (pugi::xml_node node = root.child("float"); node;
//        node = node.next_sibling("float"))
//     setFloat(node.attribute("name").as_string(),
//              node.attribute("value").as_float());
//   for (pugi::xml_node node = root.child("string"); node;
//        node = node.next_sibling("string"))
//     setString(node.attribute("name").as_string(),
...
// Print a warning message if the setting we're trying to get doesn't already
// exist in the map, then return the value in the map.
