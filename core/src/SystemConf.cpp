...
static std::set<std::string> dontRemoveValue{{"audio.device"}};
...
bool SystemConf::saveSystemConf() {
  ...
  static std::string removeID =
      "$^é(p$^mpv$êrpver$^vper$vper$^vper$vper$vper$^vperv^pervncvizn";
    ...
    for (auto &currentLine : fileLines) {
      ...
      if (idx == 0 || (idx == 1 && (fc == ';' || fc == '#'))) {
        ...
        if ((!val.empty() && val != "auto") ||
            dontRemoveValue.find(it) != dontRemoveValue.cend()) {
...
bool SystemConf::set(const std::string &name, const std::string &value) {
  if (mSystemConfFile.empty())
    return Settings::getInstance()->setString(mapSettingsName(name),
                                              value == "auto" ? "" : value);
